...
class PrescriptionModel extends EntityModel
{
  ...
  {
    $results = $this->select(
      'p.quantity, p.created_at, p.professional_id ,p.frequency_id, p.medicine_id , pr.name, pr.lastName, s.denomination as specialty, f.denomination as frequency, m.drug, m.generic_name, mt.denomination as medicine_type, mt.unit as medicine_unit',
      [
        'joins' => [
          ['type' => 'inner', 'table' => 'professional pr', 'on' => 'p.professional_id = pr.id'],
  ...
  {
    $results = $this->select(
      'p.created_at, p.quantity, p.is_active, p.professional_id ,pr.name, pr.lastname, f.denomination as frequency, mt.unit as medicine_unit',
      [
        'joins' => [
          ['type' => 'inner', 'table' => 'frequency f', 'on' => 'p.frequency_id = f.id'],
          ['type' => 'inner', 'table' => 'medicine m', 'on' => 'p.medicine_id = m.id'],
